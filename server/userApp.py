	...
	while True:
		...
		if response == 1:
			...
			while loggedIn:
				...
				while string2 == True or instruction not in range(1, 8):
					print("Invalid option entered! Please select from the following:")
					try:
						# Ask user for next step
						instruction = int(input("(1) Access Devices\n(2) Manage Devices\n(3) Change Password\n(4) Logout \n\nSelect an option: "))
						string2 = False
					except ValueError:
						string2 = True
...
def manageDevices():
	while True:
		string3 = False
		instruction2 = int(input("(1) Add Device\n(2) Remove Device\n(3) List Devices\n(4) Generate Traffic Report\n(5) Quit \nSelect an option: "))
		# Ensure user enters a valid option
		while string3 == True or instruction2 not in range(1, 6):
			print("Invalid option entered! Please select from the following:")
			try:
				# Ask user for next step
				instruction2 = int(input("(1) Add Device\n(2) Remove Device\n(3) List Devices\n(4) Generate Traffic Report\n(5) Quit \nSelect an option: "))
			...
			while deviceNameExists or len(device_name) <= 1 or len(device_name) > 15:
				if deviceNameExists:
					...
					if not deviceStillExists:
						deviceNameExists = False
				elif len(device_name) <= 1 and device_name.lower() != "q":
    ...
    with open("iotList.txt", "r") as iot_file:
        for line in iot_file:
            if f"{username}" in line:
            	temp = line.split(',')
            	#print(temp)
            	username1 = temp[0].split(":")
            	#print("split:", username1)
            	username2 = username1[1].strip()
            	print("username2:", username2)
            	print("username:", username)
            	if username == username2:
            		devices.append(line)
            	else:
            		otherDevices.append(line)
    print(devices)
    if not devices:
    	...
    	if not choice.isdigit():
    		...
    		if not choice2.isdigit():
    			...
    			with open("iotList.txt", "w") as iot_file:
    				...
    				iot_file.writelines(otherDevices)
